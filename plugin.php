...
function vccs_api_save_by_keyword() {
    ...
    if ( vccs_no_parameter_changes( $url, $keyword, $title ) ) {
        ...
        $response = array_intersect_key( $response, [ 'statusCode' => '', 'status' => '', 'message' => '' ] );

        // supply if missing field
        if (! isset( $response['statusCode'] ) ) {
            $response['statusCode'] = $response['status'] === 'success' ? 200 : 400;
        }
    }
...
function vccs_no_parameter_changes( $url, $keyword, $title ) {
    ...
    $bindings = array('url' => $url, 'keyword' => $keyword, 'title' => $title);
